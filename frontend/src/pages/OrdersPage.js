              ...
              <motion.div key={order.order_id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white dark:bg-[#3C2E26] border border-[rgba(44,30,22,0.15)] rounded-lg p-6" data-testid={`order-${order.order_id}`}>
                ...
                <div className="space-y-3 mb-4">
                  {order.items?.map((item, idx) => (
                    <div key={idx} className="bg-[#F9F4F0] dark:bg-[#2C1E16] rounded-lg px-4 py-3">
                      <div className="flex justify-between text-sm mb-1">
                        <span className="font-medium text-[#2C1E16] dark:text-[#FAFAF7]">{item.cake_name} ({item.weight})</span>
                        <span className="text-[#D0B8A8] font-semibold">₹{item.price}</span>
                      </div>
                      {item.delivery_date && (
                        <p className="text-xs text-[#E07A5F]">
                          🚚 Delivery: {new Date(item.delivery_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}
                        </p>
                      )}
                    </div>
                  ))}
